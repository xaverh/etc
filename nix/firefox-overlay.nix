...
let
  ...
  arch =
    if self.stdenv.system == "i686-linux" then "linux-i686" else "linux-x86_64";

  yearOf = with super.lib;
    yyyymmddhhmmss:
    head (splitString "-" yyyymmddhhmmss);
  monthOf = with super.lib;
    yyyymmddhhmmss:
  ...
  extractSha512Sum = sha512sums: file:
    ...
    if builtins ? split then
      substring 0 128 (head (super.lib.filter
  ...
  versionInfo = { name, version, release, system ? arch, timestamp ? null }:
    with builtins;
    if release then
    ...
    # https://download.cdn.mozilla.net/pub/firefox/releases/55.0b3/SHA256SUMS
      let
        dir =
          "https://download.cdn.mozilla.net/pub/firefox/releases/${version}";
        file = "${system}/en-US/firefox-${version}.tar.bz2";
        sha512Of = chksum: file:
          extractSha512Sum (readFile (fetchurl chksum)) file;
      in rec {
    ...
    # https://download.cdn.mozilla.net/pub/firefox/nightly/latest-mozilla-central/firefox-56.0a1.en-US.linux-x86_64.checksums
      let
        dir = if timestamp == null then
          ...
          in builtins.replaceStrings [ "/${file}" ] [ "" ]
          buildhubJSON.download.url
        else
          "https://download.cdn.mozilla.net/pub/firefox/nightly/${
            yearOf timestamp
          }/${monthOf timestamp}/${timestamp}-mozilla-central";
...
in {
  lib = super.lib // { firefoxOverlay = { inherit firefoxVersion; }; };
  ...
  devEnv = (super.shell or { }) // {
    gecko = super.callPackage ./pkgs/gecko {
      ...
      rust-cbindgen = if !(self ? "rust-cbindgen") then
        self.rust-cbindgen-latest
      else if builtins.compareVersions self.rust-cbindgen.version
      self.rust-cbindgen-latest.version < 0 then
        self.rust-cbindgen-latest
      else
        self.rust-cbindgen;
