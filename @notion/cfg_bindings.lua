...
defbindings('WScreen', {
  bdoc ('Switch to 1st object (workspace, full screen client window) within current screen.', 'ws 1'),
  kpress(META .. '1', 'WScreen.switch_nth(_, 0)'),
  bdoc ('Switch to 2nd object (workspace, full screen client window) within current screen.', 'ws 2'),
	kpress(META .. '2', 'WScreen.switch_nth(_, 1)'),
  bdoc ('Switch to 3rd object (workspace, full screen client window) within current screen.', 'ws 3'),
	kpress(META .. '3', 'WScreen.switch_nth(_, 2)'),
  bdoc ('Switch to 4th object (workspace, full screen client window) within current screen.', 'ws 4'),
	kpress(META .. '4', 'WScreen.switch_nth(_, 3)'),
  bdoc ('Switch to 5th object (workspace, full screen client window) within current screen.', 'ws 5'),
	kpress(META .. '5', 'WScreen.switch_nth(_, 4)'),
  bdoc ('Switch to 6th object (workspace, full screen client window) within current screen.', 'ws 6'),
	kpress(META .. '6', 'WScreen.switch_nth(_, 5)'),
  bdoc ('Switch to 7th object (workspace, full screen client window) within current screen.', 'ws 7'),
	kpress(META .. '7', 'WScreen.switch_nth(_, 6)'),
  bdoc ('Switch to 8th object (workspace, full screen client window) within current screen.', 'ws 8'),
	kpress(META .. '8', 'WScreen.switch_nth(_, 7)'),
  bdoc ('Switch to 9th object (workspace, full screen client window) within current screen.', 'ws 9'),
	kpress(META .. '9', 'WScreen.switch_nth(_, 8)'),
  bdoc ('Switch to 10th object (workspace, full screen client window) within current screen.', 'ws 10'),
	kpress(META .. '0', 'WScreen.switch_nth(_, 9)'),

	bdoc('Switch to next object (workspace, full screen client window) within current screen.', '->ws'),
	kpress(META .. 'grave', 'WScreen.switch_next(_)'),
	bdoc('Switch to previous object (workspace, full screen client window) within current screen.', '<-ws'),
...
-- 	}),
	...
	bdoc('Go to previous active object.', 'prev'),
...
-- 	kpress(META .. 'Tab', 'ioncore.goto_activity() or ioncore.goto_previous()'),
  ...
  kpress(ALTMETA .. 'R', 'WRegion.rqorder(_chld, "front")', '_chld:non-nil')
})

-- Client window bindings
-- These bindings affect client windows directly.
defbindings('WClientWin', {
	bdoc("Nudge the client window. This might help with some programs' resizing problems.", 'nudge'),
	kpress_wait(ALTMETA .. 'semicolon', 'WClientWin.nudge(_)'),

  bdoc('Kill client owning the client window.', 'kill'),
  kpress(ALTMETA .. 'Q', 'WClientWin.kill(_)')
})
...
--   kpress(META .. 'L', 'notioncore.exec_on(_, notioncore.lookup_script("notion-lock"))'),
  ...
  kpress(ALTMETA .. 'Return', 'mod_query.query_exec(_)'),

 bdoc('Query for Lua code to execute.', 'lua'),
 kpress(ALTMETA .. 'Control+p', 'mod_query.query_lua(_)'),

 bdoc('Query for file to edit.', 'edit'),
 kpress(META .. 'F5', 'mod_query.query_editfile(_, EDIT_COMMAND or \'run-mailcap --action=edit\')'),

 bdoc('Query for file to view.', 'view'),
 kpress(META .. 'F6', 'mod_query.query_runfile(_, VIEW_COMMAND or \'xdg-open\')'),

 bdoc('Query for keybinding.', 'qkb'),
 kpress(META .. 'F7', 'mod_query.query_binding(_, _sub)'),

 bdoc('Query for workspace to go to or create a new one.', '+ws'),
 kpress(ALTMETA .. 'F9', 'mod_query.query_workspace(_)'),

 bdoc('Query for a client window to go to.', 'go'),
 kpress(ALTMETA .. 'p', 'mod_query.query_gotoclient(_)'),
 bdoc('Display context menu.', 'ctx'),
 kpress(META .. 'M', 'mod_menu.menu(_, _sub, \'ctxmenu\')'),
 bdoc('Query for context menu.', 'qctx'), kpress(ALTMETA .. 'M',
...
defbindings('WFrame', {
 ...
 kpress(META .. 'Y', 'WFrame.maximize_horiz(_)'),
 bdoc('Maximize the frame vertically.', 'vmax'),
 kpress(META .. 'X', 'WFrame.maximize_vert(_)'), bdoc('Display context menu.'),
 ...
 mclick('Button2@tab', 'WFrame.p_switch_tab(_)'), bdoc('Resize the frame.'),
 mdrag('Button1@border', 'WFrame.p_resize(_)'),
 mdrag(META .. 'Button3', 'WFrame.p_resize(_)'), bdoc('Move the frame.'),
...
defbindings('WFrame.floating', {
 ...
 bdoc('Move the frame.'), mdrag('Button1@tab', 'WFrame.p_move(_)')
})
...
defmenu('mainmenu', {
 ...
 submenu('Styles', 'stylemenu'), submenu('Session', 'sessionmenu')
})
...
defctxmenu('WFrame', 'Frame', {
	...
	menuentry('Close', 'WRegion.rqclose_propagate(_, _sub)'), -- Low-priority entries
